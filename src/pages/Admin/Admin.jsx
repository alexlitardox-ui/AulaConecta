...
export default function Admin() {
  ...
  async function handleReview(id, status) {
    ...
    try {
      ...
      setData((current) => ({ ...current, materials: current.materials.map((item) => item.id === id ? { ...item, review_status: status } : item), stats: { ...current.stats, pendingMaterials: Math.max(0, current.stats.pendingMaterials + (status === "pending" ? 1 : -1)) } }))
