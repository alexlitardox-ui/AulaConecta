...
function Avatar({ user }) {
  ...
  return (
    <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-blue-600 to-indigo-600 font-bold text-white shadow-sm ring-2 ring-white">
      {getInitials(user)}
    </div>
  )
}
...
function ConversationList({
  ...
  return (
    <aside className="flex h-full flex-col border-r border-slate-200 bg-white">
      <div className="border-b border-slate-200 bg-gradient-to-br from-white via-white to-blue-50/70 p-5">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              Coordina tutorías, solicitudes y grupos.
            </p>
          </div>
        ...
        <div className="mt-4 flex items-center justify-between text-xs text-slate-500">
          ...
          {searchTerm && <span>{filteredConversations.length} resultados</span>}
        </div>
      </div>
        ...
        {filteredConversations.length === 0 ? (
          <div className="flex min-h-72 flex-col items-center justify-center px-5 text-center">
            ...
            <h2 className="mt-4 font-bold text-slate-900">
              {searchTerm ? "No encontramos resultados" : "Sin conversaciones"}
            </h2>

            <p className="mt-2 text-sm leading-6 text-slate-500">
              {searchTerm
                ? "Prueba con otro nombre o una palabra del mensaje."
                : "Cuando conectes con otro estudiante, el chat aparecerá aquí."}
            </p>
          </div>
        ) : (
            ...
            {filteredConversations.map((conversation) => {
              ...
              return (
                <button
                  ...
                  className={`group relative flex w-full items-center gap-3 overflow-hidden rounded-2xl border p-3 text-left transition-all duration-200 ${
                    isSelected
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className={`mt-1 truncate text-sm ${isSelected ? "text-blue-700" : "text-slate-500"}`}>
